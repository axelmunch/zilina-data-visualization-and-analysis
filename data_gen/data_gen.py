...
def wait_for_influxdb(max_retries=10, delay=5):
    for attempt in range(max_retries):
        try:
            ...
            print(f"⏳ Attempt {attempt + 1}/{max_retries}: InfluxDB not ready yet ({e})")
        time.sleep(delay)
...
def generate_data(device_id):
    ...
    data.append(Point("acceleration")
...
try:
    while True:
        ...
        print(f"✅ Wrote data for {len(devices)} devices at {datetime.now(timezone.utc).isoformat()}")
