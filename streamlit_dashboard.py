...
def load_data(source: str) -> pd.DataFrame:
    ...
    if source == "csv":
        ...
        try:
            ...
            st.error("Package `influxdb-client` not installed. Add it to your Dockerfile / requirements.")
            st.stop()
...
def main():
    ...
    selected_types = st.sidebar.multiselect("Sensor types", sensor_types, default=sensor_types)
    ...
    start, end = st.sidebar.slider(
        "Time range", min_value=min_time, max_value=max_time, value=(min_time, max_time), format="YYYY‑MM‑DD HH:mm:ss"
    )
